@php
    ...
    $checktruefalse[0]->homesidebar_block;
    $checktruefalse[0]->widgets_block;
@endphp
<!DOCTYPE html>
<html lang="en">
   <head>
      ...
      @endif
      {{-- Font --}}
      <link href="{{asset('themes/theme1/assests/fonts/css/font-awesome.min.css')}}" rel="stylesheet" type="text/css">
      <script src="https://kit.fontawesome.com/a2189951ff.js" crossorigin="anonymous"></script>
      <!-- Bootstrap core CSS -->
      <link href="{{asset('themes/theme1/assests/css/bootstrap.min.css')}}" rel="stylesheet"> 
      <link href="{{asset('themes/theme1/assests/css/carousel.css')}}" rel="stylesheet">       
      <meta name="csrf-token" content="{{ csrf_token() }}">
      <link href="{{asset('themes/theme1/assests/css/custom.css')}}" rel="stylesheet">
...
<div class="header">
    ...
    <video playsinline="playsinline" autoplay="autoplay" muted="muted" loop="loop">
      <source src="{{asset('themes/theme1/assests/coverr.mp4')}}" type="video/mp4">
    </video>
...
<div class="container">
  ...
  <div class="row">
    <div class="col-md-12 text-right small" style="margin-top: -40px;">
      ...
      <a class="text-warning" href="{{url('moreFeaturedArticle')}}">More Featured >>></a>
    </div>
  </div>
  <div class="row">
...
$articleview = Article::Where([['delete_status','=',1],['draft','=',1]])->with('user','mediaLibrary')->orderBy('id','desc')->whereHas('user', function($query) {
...
<div class="container">
  <div class="row">
    <div class="col-md-12">
      <h3 class="block-title mt-5 mb-4">Main Posts</h3>
    </div>
{{-- this line of $checktruefalse[0]->homesidebar_block===1 is used for spread the div start --}}
    @if($checktruefalse[0]->homesidebar_block===1)
    <div class="col-md-9">
    @else
    <div class="col-md-12">
    @endif
{{-- this line of $checktruefalse[0]->homesidebar_block===1 is used for spread the div End Here --}}
      ...
      <div class="latest-post mb-4">
        ...
        <a href="{{url('viewArticle/'.$article['aslug'])}}"><img src="{{url('uploads/media/'.$article['mediaLibrary']['original_filename'])}}" class="img-fluid" alt="..."></a>
{{--         @else This else condition is used for default image.
        <a href="{{url('viewArticle/'.$article['aslug'])}}">
        <img class="img-fluid" src="{{url('uploads/BlogImage/No-Image-Available-article.png')}}" alt=""></a> --}}
        @endif  
...
{{-- Home sidebar ON/OFF Start --}}@if($checktruefalse[0]->homesidebar_block===1)
    <div class="col-md-3">
      {{-- Search ON/OFF --}}
      ...
      @endif
      {{-- Search ON/OFF end --}}
      {{-- Welcome note include directory under theme1 --}}
...
@foreach($data as $data)
      <aside id="text-2" class="widget widget_text mb-4">
        ...
        <div class="textwidget span4 collapse-group">
          <img alt="avatar" src="{{$data->image}}" class="float-right">
          <p>
            {{Str::substr($data->description,0,129)}}
            <span class="collapse" id="viewdetails3">{{Str::substr($data->description,129)}}</span>
            <a class="text-warning" data-toggle="collapse" data-target="#viewdetails3">More... &raquo;</a>
          </p>
        </div>
      </aside>
@endforeach
@endif --}}
@include('theme1.includes.welcome_note')
{{-- Welcome note include directory under theme1 --}}
      ...
      <aside id="text-3" class="widget widget_text mb-4">
        ...
        <a href="{{url('category')}}"><p class="text-right text-warning">All categories</p></a>
      </aside>
      @endif {{-- End ON/Off --}}
    </div>
    @endif{{-- ON/OFF homesidebar end --}}
  </div>
</div>
<!-- end below header part--> 

@if($checktruefalse[0]->widgets_block===1)
  @include('theme1.includes.widgets')
@endif
{{-- above widgets code move into theme includes directory --}}


{{-- This widgets comes from theme1 include directory --}}

<!-- Scripting Part And Footerpart-->
  @include('theme1.includes.mainFooter')
<script src="{{asset('themes/theme1/assests/js/jquery-3.4.1.slim.min.js')}}"></script>
<script src="{{asset('themes/theme1/assests/js/bootstrap.bundle.min.js')}}"></script>
...
<script type="text/javascript">
  var new_ticket_widget='<?php echo url("/new-ticket-widget")?>';
  $(function(){
  //alert(new_ticket_widget);
  $.ajaxSetup({
    headers: {
  ...
  $("#createticket").submit(function(e){
    ...
    $.ajax({
      type:'POST',
      url:new_ticket_widget,
      data:{name:name, lname:lname, email:email,subject:subject,
